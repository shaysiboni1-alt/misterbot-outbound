...
async function loadSSOT(force = false) {
  ...
  try {
    ...
    vrs = await batchGetRanges(["SETTINGS!A:B", "PROMPTS!A:B", "INTENTS!A:F"]);
  }
...
async function updateLeadRowByLeadId(leadId, patch) {
  ...
  const { sheets, sheetId } = await getSheetsClient();
  const getResp = await sheets.spreadsheets.values.get({ spreadsheetId: sheetId, range: "OUTBOUND_LEADS!A:Z" });
  ...
  const rowIndex = values.findIndex((r, idx) => idx > 0 && String(r?.[leadIdIdx] || "").trim() === leadIdStr);
